...
class OlnRPNHead(RPNHead):
    ...
    def forward_single(self, x):
        ...
        # x = x.view(N, C, -1)
        # attn = torch.bmm(x.transpose(-2,-1), x)
        # attn = F.softmax(attn, dim=-1)
        # rpn_objectness_pred = rpn_objectness_pred.view(N, C, -1)
        # rpn_objectness_pred = torch.bmm(attn, rpn_objectness_pred.transpose(-2, -1))
        # rpn_objectness_pred = rpn_objectness_pred.transpose(-2, -1)
        # rpn_objectness_pred = rpn_objectness_pred.view(N, C, H, W)
    ...
    def _get_targets_single(self,
        ...
        if len(objectness_pos_inds) > 0:
            ...
            # objectness_weights[objectness_pos_inds] = pos_objectness_targets
    ...
    def _get_bboxes_single(self,
        ...
        for idx in range(len(cls_scores)):
            ...
            # scores = torch.stack([rpn_objectness_scores,rpn_cls_scores], dim=1)
            # thresh = 0.005
            # rpn_cls_scores = torch.sign(rpn_cls_scores-thresh).float()
            # scores = rpn_objectness_scores * rpn_cls_scores
            # alpha = 0.0
            # import pdb;pdb.set_trace()
            # scores = alpha * rpn_objectness_scores + (1 - alpha) * rpn_cls_scores
            # scores = rpn_cls_scores
            # scores = torch.sqrt(rpn_cls_scores*rpn_objectness_scores)
