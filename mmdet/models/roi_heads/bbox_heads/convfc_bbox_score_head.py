...
class ConvFCBBoxScoreHead(ConvFCBBoxHead):
    ...
    def __init__(self, 
        ...
        # change cls branch to fit focal loss
    ...
    def forward(self, x):
        ...
        return cls_score, bbox_pred, bbox_score, feat_cls, feat_loc
    ...
    def loss(self,
        ...
        # disentangle rrgularizaion
        # feat_cls = feat_cls[pos_inds]
        # feat_loc = feat_loc[pos_inds]
        # cosine_similarity = F.cosine_similarity(feat_cls, feat_loc)
        # orthogonality = cosine_similarity.abs().mean()
        # losses['loss_orthogonality'] = orthogonality * 10.0
        return losses
    ...
    def get_bboxes(self,
        ...
        # output all predict boxes in rpn and roi
        # rois = rois[:, 1:] / scale_factor
        # return {'rpn_boxes': rois.detach().cpu(), 'rpn_scores': rpn_score.detach().cpu(), \
        #         'roi_boxes': bboxes.detach().cpu(), 'roi_scores': bbox_score.sigmoid().detach().cpu()}, 0
        ...
        # bbox_score = torch.sqrt(cls_score.sigmoid() * bbox_score.sigmoid())
        # bbox_score = cls_score.sigmoid()
        # scores = torch.sqrt(rpn_score * bbox_score)
